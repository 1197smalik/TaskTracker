"""Tests for owner-based work item detail visibility filtering."""
...
def test_work_item_detail_requires_authenticated_principal(
    visibility_client: TestClient,
) -> None:
    response = visibility_client.get("/api/v1/projects/project-1/work-items/work-item-1")

    assert response.status_code == 401
    assert response.json()["detail"]["error_code"] == "missing_bearer_token"


def test_work_item_detail_returns_owned_project_item(
    visibility_client: TestClient,
) -> None:
    response = visibility_client.get(
        "/api/v1/projects/project-1/work-items/work-item-1",
...
def test_work_item_detail_denies_inaccessible_project(
    visibility_client: TestClient,
) -> None:
    response = visibility_client.get(
        "/api/v1/projects/project-2/work-items/work-item-2",
        headers={"Authorization": f"Bearer {create_access_token('user-owner-1')}"},
    )
...
def test_work_item_detail_returns_not_found_for_missing_work_item(
    visibility_client: TestClient,
) -> None:
    response = visibility_client.get(
        "/api/v1/projects/project-1/work-items/missing-work-item",
        headers={"Authorization": f"Bearer {create_access_token('user-owner-1')}"},
    )

    assert response.status_code == 404
    assert response.json()["error_code"] == "work_item_not_found"
